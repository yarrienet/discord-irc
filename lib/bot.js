...
class Bot {
  constructor(options) {
    ...
    this.nickMapping = options.nickMapping;
    this.avatarMapping = options.avatarMapping;
  ...
  attachListeners() {
    this.discord.on('ready', () => {
      ...
      //   logger.info("Updating Discord members for channel: " + channel);
      //   // force update the members cache to get the avatar

      //   // TODO: this doesn't work... update to discord v14 needed? Error [GUILD_MEMBERS_TIMEOUT]: Members didn't arrive in time.
      //   const guildMembers = this.findDiscordChannel(channel).guild.members.fetch()
      //   .then(() => { console.log("Updated Discord members for channel: " + channel) })
  ...
  getDiscordAvatar(nick, channel) {
    ...
    if (nick in this.avatarMapping) {
      return this.avatarMapping[nick]
    }

    let users;
    if (nick in this.nickMapping) {
      users = guildMembers.filter(member => member.user.id === this.nickMapping[nick] )
    } else {
      // Resort to finding using username/nick
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (webhook) {
      ...
      let username = author
      if (author in this.nickMapping) {
        const guildMembers = discordChannel.guild.members.cache;
        const users = guildMembers.filter(member => member.user.id === this.nickMapping[author] )
        if (users && users.size == 1) {
          const member = users.first()
          // TODO: update to discord.js v14 to support member.user.displayName
          username = member.nickname || username
        }
      }

      username = _.padEnd(username.substring(0, USERNAME_MAX_LENGTH), USERNAME_MIN_LENGTH, '_');
